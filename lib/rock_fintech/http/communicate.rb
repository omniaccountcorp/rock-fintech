  ...
  module Http
    ...
    def self.post(service, params, config, post_path, version=VERSION)
      ...
      begin
        ...
        RockFintech.logger.info "[#{service}] 返回的报文为：\n#{response.body.force_encoding('utf-8')}"

        if response.is_a?(Net::HTTPSuccess)
          response_raw_body = unpack_body(response.body, config)
          result = RockFintech::Http::Response.new(service: service,
      ...
      RockFintech.logger.info "[#{service}] 返回结果为：\n#{result}\n\n"
    ...
    # 正确的返回字符串例子：
    #    'version=1.0&service=RockFintech.trust.onekeyContract&signType=0&sign=cdab41a05d595d6a5f5a818af2b39398&resData={"contracts":"RB1711167FYG9U29","userName":"王五","userIdentity":"330602198711160034","userMobile":"18357101332","orderNo":"5a0d488fe0c016049a000001","resultCode":"0007","processTime":"2017-11-16 16:13:04"}'
    #
    # 异常的返回字符串例子：
    #    '{"orderNo":"FFSS1","errorCode":"0107","service":"RockFintech.trust.onekeyContract","errorMsg":"姓名不能为空"}'
